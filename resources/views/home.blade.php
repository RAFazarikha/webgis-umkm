...
<section class="max-w-7xl mx-auto px-6 py-16 text-center">
    ...
    <div class="grid md:grid-cols-3 gap-8 mt-8 mb-6">
        @foreach ($topumkm as $umkm)
            <x-culinary-card :title="$umkm->nama_usaha" :location="$umkm->subdistrict->name" :alamat="$umkm->alamat" :kategori="$umkm->kategori" :tags="['Rating: ' . ($umkm->rating ?? '-'), 'Jam Operasional: ' . ($umkm->jam_operasional ?? '-')]" />
        @endforeach
    </div>
